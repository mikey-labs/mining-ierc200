 ...
 * @returns {Object}
 */
export const getHeaders = (method, path, params = {}) => {
  let _time = timestamp(),
    _sign;
  if (method === 'GET') {
    let arr = [];
 ...
 * @returns {Object}
 */
export async function get_api_v5_public_instruments({ instType, uly, instId }) {
  return await http(
    cleanObj({
      instType: instType,
      uly: uly,
...
export async function get_api_v5_account_positions({ instType, instId, posId }) {
  return await http(
    cleanObj({
      instType: instType,
      instId: instId,
 ...
 * @returns {Object}
 */
export async function get_api_v5_account_balance({ ccy }) {
  return await http(
    cleanObj({
      ccy: ccy
    })
 ...
 * @returns {Object}
 */
export async function get_api_v5_account_accountPositionRisk({ instType }) {
  return await http(
    cleanObj({
      instType: instType
    })
 ...
 * @returns {Object}
 */
export async function get_api_v5_account_config() {
  return await http();
}
 ...
 * @returns {Object}
 */
export async function post_api_v5_account_setPositionMode({ posMode }) {
  return await http(
    cleanObj({
      posMode: posMode
    })
 ...
 * @returns {Object}
 */
export async function post_api_v5_account_setLeverage({ instId, ccy, lever, mgnMode, posSide }) {
  return await http(
    cleanObj({
      instId: instId,
      ccy: ccy,
 ...
 * @returns {Object}
 */
export async function post_api_v5_trade_order({
  instId,
  tdMode,
  ccy,
  clOrdId,
